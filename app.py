...
class WordApp(Qw.QMainWindow):
    def __init__(self):
        ...
        self.setWindowTitle("English Vocabulary App")  #タイトル設定
        rect = Qc.QRect()
    ...
    def study_words(self):
        ...
        for word, details in self.words.items():
            word_widget = WordWidget(word, details["意味"], details["例文"])
    ...
    def show_answer(self):
        if self.current_quiz_index < len(self.quiz_words):
            word, details = self.quiz_words[self.current_quiz_index]
            self.quiz_answer_label.setText(f"意味: {details['意味']}\nExample: {details['例文']}")
